...
@Repository
public class CargoQueryRepository {

    private final DatabaseClient db;
    private static final Set<String> SORTABLE = Set.of("codigo", "nombre", "sueldo_basico", "created_at");

    public CargoQueryRepository(DatabaseClient db) {
        this.db = db;
    }

    public Mono<Boolean> existsByCodigo(String codigo, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM cargo WHERE codigo=:c AND empresa_id=:e AND deleted_at IS NULL")
    ...
    public Mono<Boolean> existsByCodigoExcludingId(String codigo, Long id, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM cargo WHERE codigo=:c AND empresa_id=:e AND id<>:id AND deleted_at IS NULL")
            .bind("c", codigo).bind("e", empresaId).bind("id", id)
            .map(row -> ((Number) row.get("c")).longValue()).one().map(c -> c > 0);
    }

    public Mono<Boolean> existsActivoEnEmpresa(Long id, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM cargo WHERE id=:id AND empresa_id=:e AND deleted_at IS NULL")
            .bind("id", id).bind("e", empresaId)
            .map(row -> ((Number) row.get("c")).longValue()).one().map(c -> c > 0);
    }

    public Mono<CargoResponseDto> findActiveById(Long id, Long empresaId) {
        return db.sql("""
                SELECT c.id AS "id", c.codigo AS "codigo", c.nombre AS "nombre", c.nivel_cargo AS "nivelCargo",
                       c.grado AS "grado", c.tipo_remuneracion AS "tipoRemuneracion",
                       c.sueldo_basico AS "sueldoBasico", c.sueldo_maximo AS "sueldoMaximo",
                       c.mision AS "mision", c.descripcion AS "descripcion",
                       c.activo AS "active", c.created_at AS "createdAt"
                FROM cargo c WHERE c.id=:id AND c.empresa_id=:e AND c.deleted_at IS NULL LIMIT 1
                """)
            .bind("id", id).bind("e", empresaId)
            .fetch().one()
            .map(row -> MapperRepository.mapResultSetToObject(row, CargoResponseDto.class));
    }

    @SuppressWarnings("unchecked")
    public Mono<PageResponseDto<CargoTableDto>> list(PageableDto<?> request, Long empresaId) {
        ...
        StringBuilder sql = new StringBuilder("""
                SELECT c.id AS "id", c.codigo AS "codigo", c.nombre AS "nombre", c.nivel_cargo AS "nivelCargo",
                       c.sueldo_basico AS "sueldoBasico", c.activo AS "active", COUNT(*) OVER() AS total_rows
                FROM cargo c WHERE c.empresa_id=:e AND c.deleted_at IS NULL
                """);
        ...
        return spec.fetch().all().collectList().map(rows -> {
            ...
            List<CargoTableDto> content = rows.stream()
                .map(r -> MapperRepository.mapResultSetToObject((Map<String, Object>) r, CargoTableDto.class))
                .toList();
