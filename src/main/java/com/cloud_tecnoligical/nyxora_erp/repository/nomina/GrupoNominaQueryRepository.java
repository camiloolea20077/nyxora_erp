...
@Repository
public class GrupoNominaQueryRepository {

    private final DatabaseClient db;
    private static final Set<String> SORTABLE = Set.of("codigo", "nombre", "created_at");

    public GrupoNominaQueryRepository(DatabaseClient db) {
        this.db = db;
    }

    public Mono<Boolean> existsByCodigo(String codigo, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM grupo_nomina WHERE codigo=:c AND empresa_id=:e AND deleted_at IS NULL")
    ...
    public Mono<Boolean> existsByCodigoExcludingId(String codigo, Long id, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM grupo_nomina WHERE codigo=:c AND empresa_id=:e AND id<>:id AND deleted_at IS NULL")
            .bind("c", codigo).bind("e", empresaId).bind("id", id)
            .map(row -> ((Number) row.get("c")).longValue()).one().map(c -> c > 0);
    }

    public Mono<Boolean> existsActivoEnEmpresa(Long id, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM grupo_nomina WHERE id=:id AND empresa_id=:e AND deleted_at IS NULL")
            .bind("id", id).bind("e", empresaId)
            .map(row -> ((Number) row.get("c")).longValue()).one().map(c -> c > 0);
    }

    public Mono<GrupoNominaResponseDto> findActiveById(Long id, Long empresaId) {
        return db.sql("""
                SELECT g.id AS "id", g.codigo AS "codigo", g.nombre AS "nombre",
                       g.frecuencia_pago AS "frecuenciaPago", g.activo AS "active", g.created_at AS "createdAt"
                FROM grupo_nomina g WHERE g.id=:id AND g.empresa_id=:e AND g.deleted_at IS NULL LIMIT 1
                """)
            .bind("id", id).bind("e", empresaId)
            .fetch().one()
            .map(row -> MapperRepository.mapResultSetToObject(row, GrupoNominaResponseDto.class));
    }

    @SuppressWarnings("unchecked")
    public Mono<PageResponseDto<GrupoNominaTableDto>> list(PageableDto<?> request, Long empresaId) {
        ...
        StringBuilder sql = new StringBuilder("""
                SELECT g.id AS "id", g.codigo AS "codigo", g.nombre AS "nombre",
                       g.frecuencia_pago AS "frecuenciaPago", g.activo AS "active", COUNT(*) OVER() AS total_rows
                FROM grupo_nomina g WHERE g.empresa_id=:e AND g.deleted_at IS NULL
                """);
        if (search != null && !search.isEmpty()) {
            sql.append(" AND (LOWER(g.nombre) LIKE LOWER(:search) OR LOWER(g.codigo) LIKE LOWER(:search)) ");
        }
        sql.append(" ORDER BY g.").append(orderBy).append(" ").append(order);
        ...
        return spec.fetch().all().collectList().map(rows -> {
            ...
            List<GrupoNominaTableDto> content = rows.stream()
                .map(r -> MapperRepository.mapResultSetToObject((Map<String, Object>) r, GrupoNominaTableDto.class))
                .toList();
