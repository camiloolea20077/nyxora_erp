...
@Repository
public class FaltaQueryRepository {

    private final DatabaseClient db;
    private static final Set<String> SORTABLE = Set.of("codigo", "nombre", "created_at");

    public FaltaQueryRepository(DatabaseClient db) {
        this.db = db;
    }

    public Mono<Boolean> existsByCodigo(String codigo, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM falta WHERE codigo=:c AND empresa_id=:e AND deleted_at IS NULL")
    ...
    public Mono<Boolean> existsByCodigoExcludingId(String codigo, Long id, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM falta WHERE codigo=:c AND empresa_id=:e AND id<>:id AND deleted_at IS NULL")
            .bind("c", codigo).bind("e", empresaId).bind("id", id)
            .map(row -> ((Number) row.get("c")).longValue()).one().map(c -> c > 0);
    }

    public Mono<Boolean> existsActivoEnEmpresa(Long id, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM falta WHERE id=:id AND empresa_id=:e AND deleted_at IS NULL")
            .bind("id", id).bind("e", empresaId)
            .map(row -> ((Number) row.get("c")).longValue()).one().map(c -> c > 0);
    }

    public Mono<FaltaResponseDto> findActiveById(Long id, Long empresaId) {
        return db.sql("""
                SELECT f.id AS "id", f.clasificacion_falta_id AS "clasificacionFaltaId", f.codigo AS "codigo",
                       f.nombre AS "nombre", f.descripcion AS "descripcion", f.caducidad_dias AS "caducidadDias",
                       f.politica AS "politica", f.activo AS "active", f.created_at AS "createdAt"
                FROM falta f WHERE f.id=:id AND f.empresa_id=:e AND f.deleted_at IS NULL LIMIT 1
                """)
            .bind("id", id).bind("e", empresaId)
            .fetch().one()
            .map(row -> MapperRepository.mapResultSetToObject(row, FaltaResponseDto.class));
    }

    @SuppressWarnings("unchecked")
    public Mono<PageResponseDto<FaltaTableDto>> list(PageableDto<?> request, Long empresaId) {
        ...
        StringBuilder sql = new StringBuilder("""
                SELECT f.id AS "id", f.codigo AS "codigo", f.nombre AS "nombre", f.activo AS "active",
                       COUNT(*) OVER() AS total_rows
                FROM falta f WHERE f.empresa_id=:e AND f.deleted_at IS NULL
                """);
        ...
        return spec.fetch().all().collectList().map(rows -> {
            ...
            List<FaltaTableDto> content = rows.stream()
                .map(r -> MapperRepository.mapResultSetToObject((Map<String, Object>) r, FaltaTableDto.class))
                .toList();
