...
public class MarcaQueryRepository {
    ...
    public MarcaQueryRepository(DatabaseClient db) {
        this.db = db;
    }

    public Mono<Boolean> existsByCodigo(String codigo, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM marca WHERE codigo=:c AND empresa_id=:e AND deleted_at IS NULL")
    ...
    public Mono<Boolean> existsByCodigoExcludingId(String codigo, Long id, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM marca WHERE codigo=:c AND empresa_id=:e AND id<>:id AND deleted_at IS NULL")
            .bind("c", codigo).bind("e", empresaId).bind("id", id)
            .map(row -> ((Number) row.get("c")).longValue()).one().map(c -> c > 0);
    }

    public Mono<MarcaResponseDto> findActiveById(Long id, Long empresaId) {
        return db.sql("""
                SELECT m.id AS "id", m.codigo AS "codigo", m.nombre AS "nombre",
                       m.activo AS "active", m.created_at AS "createdAt"
                FROM marca m WHERE m.id=:id AND m.empresa_id=:e AND m.deleted_at IS NULL LIMIT 1
                """)
            .bind("id", id).bind("e", empresaId)
            .fetch().one()
            .map(row -> MapperRepository.mapResultSetToObject(row, MarcaResponseDto.class));
    }

    @SuppressWarnings("unchecked")
    public Mono<PageResponseDto<MarcaTableDto>> list(PageableDto<?> request, Long empresaId) {
        ...
        StringBuilder sql = new StringBuilder("""
                SELECT m.id AS "id", m.codigo AS "codigo", m.nombre AS "nombre", m.activo AS "active",
                       COUNT(*) OVER() AS total_rows
                FROM marca m WHERE m.empresa_id=:e AND m.deleted_at IS NULL
                """);
        ...
        return spec.fetch().all().collectList().map(rows -> {
            ...
            List<MarcaTableDto> content = rows.stream()
                .map(r -> MapperRepository.mapResultSetToObject((Map<String, Object>) r, MarcaTableDto.class))
                .toList();
