...
@Repository
public class CpcQueryRepository {

    private final DatabaseClient db;
    private static final Set<String> SORTABLE = Set.of("codigo", "nombre", "created_at");

    public CpcQueryRepository(DatabaseClient db) {
        this.db = db;
    }

    public Mono<Boolean> existsByCodigo(String codigo, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM cpc WHERE codigo=:c AND empresa_id=:e AND deleted_at IS NULL")
    ...
    public Mono<Boolean> existsByCodigoExcludingId(String codigo, Long id, Long empresaId) {
        return db.sql("SELECT count(*) AS c FROM cpc WHERE codigo=:c AND empresa_id=:e AND id<>:id AND deleted_at IS NULL")
            .bind("c", codigo).bind("e", empresaId).bind("id", id)
            .map(row -> ((Number) row.get("c")).longValue()).one().map(c -> c > 0);
    }

    public Mono<CpcResponseDto> findActiveById(Long id, Long empresaId) {
        return db.sql("""
                SELECT c.id AS "id", c.vigencia_id AS "vigenciaId", c.cpc_padre_id AS "cpcPadreId",
                       c.codigo AS "codigo", c.nombre AS "nombre", c.maneja_movimiento AS "manejaMovimiento",
                       c.activo AS "active", c.created_at AS "createdAt"
                FROM cpc c WHERE c.id=:id AND c.empresa_id=:e AND c.deleted_at IS NULL LIMIT 1
                """)
            .bind("id", id).bind("e", empresaId)
            .fetch().one()
            .map(row -> MapperRepository.mapResultSetToObject(row, CpcResponseDto.class));
    }

    @SuppressWarnings("unchecked")
    public Mono<PageResponseDto<CpcTableDto>> list(PageableDto<?> request, Long empresaId) {
        ...
        StringBuilder sql = new StringBuilder("""
                SELECT c.id AS "id", c.cpc_padre_id AS "cpcPadreId", c.codigo AS "codigo", c.nombre AS "nombre",
                       c.maneja_movimiento AS "manejaMovimiento", c.activo AS "active", COUNT(*) OVER() AS total_rows
                FROM cpc c WHERE c.empresa_id=:e AND c.deleted_at IS NULL
                """);
        ...
        return spec.fetch().all().collectList().map(rows -> {
            ...
            List<CpcTableDto> content = rows.stream()
                .map(r -> MapperRepository.mapResultSetToObject((Map<String, Object>) r, CpcTableDto.class))
                .toList();
